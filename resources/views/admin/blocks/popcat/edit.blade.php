...
@section('script')
    ...
    <script src="{{ asset('adminka/js/galleries/popcat_gallery.js')}}"></script>
    <script>
        //progress bars
        ...
        <div class="container-fluid">
            ...
            <div class="row clearfix">
                <div class="col-md-12">
                    @if($gallery_id)
                    <div class="card">
                        <form method="POST" action="{{ route('admin.galleries.update', ['gallery' => $gallery_id ]) }}" class="" id="form_admin_sidebar_update" enctype="multipart/form-data" >
                            {{--                        <form method="POST" action="{{ route('admin.sidebars.update', ['sidebar' => 1 ]) }}" class="" id="form_admin_sidebar_update"  >--}}
                            @csrf
                            @method("PATCH")
                            <div class="body" id="main_gallery_wrap">
                                <div id="main_gallery" class="list-unstyled row clearfix">
                                    @forelse ($gallery ?? [] as $key =>$image)
                                        ...
                                        <div id="image-{{$key}}"  data-id="{{ $key.$img }}" class="col-lg-2 col-md-4 col-sm-12 m-50 card border-secondary js_images_counter">
                                            ...
                                            <div class="card-body text-secondary" data-parent_id="{{$key}}">
                                                <input type="file" name="image[]" class="dropify" id="main_image-{{$key}}" data-input-id="{{$key}}" data-default-file="{{ $img !==null ? asset('storage/galleries/'.$img) : ''}}" >
                                                <input type="hidden" name="old_image[]" value="{{$img}}" id="old_image_{{$key}}" >
                                            </div>
                                            <ul class="list-group list-group-flush">
                                                <li class="list-group-item">
                                                    <label for="title[]">Title:</label>
                                                    <textarea class="container-fluid" id="title[]" name="title[]">{{ $image['title'] ?? null }}</textarea>
                                                </li>
                                                <li class="list-group-item">
                    ...
                    @else
                        <h5>No Categories added! Please, <a href="{{ route('admin.categories.index') }}">create banner for category</a> and <a href="{{ route('admin.banners.index') }}">add banner</a> to block</h5>
                    @endif
