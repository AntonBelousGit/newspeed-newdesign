<div class="bg-Cber">
    <div class="mailing container">
        <figure class="mailing-item spase-b-c">
            <picture>
                <source srcset="{{ asset('dist/img/subscribe1.webp')}} " type="image/webp">
                <img src="{{ asset('dist/img/subscribe1.png')}}" alt=""></picture>
            <figcaption class="text-b text-b-m-serch l-s">
                <div>Подписаться</div>
                на нашу рассылку
            </figcaption>
        </figure>
        <form class="mailing-form spase-b-c form-s">
            <input class="form-s-search" type="email" placeholder="Введите ваш e-mail">
            <button class="btn btn-l btn-orange">Подписаться</button>
        </form>
    </div>
</div>
